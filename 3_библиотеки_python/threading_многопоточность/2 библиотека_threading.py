...
if q == 2:
    # Создание Background потока
    # daemon True позволяет создать не главный поток
    thread = threading.Thread(target=func, args=(1, 3_000_000), daemon=True)
    # Как только Foreground поток закончился, программа завершилась не дожидаясь
    thread.start()
    # join строит мост между главным потоком и Background потоком
    thread.join()
    # Join подцепляет второстепенные потоки к главному
    print("завершение потока main")

# Параллельные потоки работают медленнее последовательных из-за context switch
